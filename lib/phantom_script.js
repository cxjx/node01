...
page.onResourceReceived = function(response) {
  ...
  if (imageContentTypes.indexOf(contentType) != -1) {
    if (debug) console.log('[debug] Receive', contentType, response.url);//, JSON.stringify(response.headers, undefined, 4));
    images.push( {
...
//   if (debug) console.log('[debug] Timeout', e.errorCode, e.errorCode, e.url);
//   phantom.exit(1);
// };
